<?php # registro_boleto.php 
		# Hector Miranda

$page_title = ':: ECCO - Registro de boletos ::';
...
if (isset($_POST['submitted'])) {
	...
	// revisa el boleto:
	if (empty($_POST['clave_boleto'])) {
		$errores[] = 'Olvidaste introducir el boleto.';
	}else{                
		$v3 = $_POST['clave_boleto']; //valor de la clave del boleto
		
		$v1 = "select * from boleto where clave_boleto='$v3'"; //coincidencias con el boleto
		
		$v2 = @mysqli_query($dbc, $v1); //resultados de la consulta de coincidencias
	  
		if (mysqli_num_rows($v2) != 0) { //si existe alguna coincidencia
			$errores[] = 'El n&uacute;mero de boleto ya est&aacute; registrado.';
		}else {
			$clave_boleto = mysqli_real_escape_string($dbc, trim($_POST['clave_boleto'])); 
		}
	}
	...
	if (empty($errores)) { // Si todo está bien...
		...
		// Registra el boleto en la base de datos...
		
		// Realiza la consulta:
		$consulta = "INSERT INTO boleto (clave_boleto, nombre, telefono, correo, escuela, matricula) 
				VALUES ('$clave_boleto', '$nombre', '$telefono', '$correo', '$escuela', '$matricula')";
		$resultado = @mysqli_query ($dbc, $consulta); // Run the query.
		if ($resultado) { // If it ran OK.
		
			// Imprime un mensaje:
			echo '<h1>&iexcl;Registro exitoso!</h1>
					<p>Se registr&oacute; el boleto <b>#', $clave_boleto, '</b> con la siguiente informaci&oacute;n:</p>
...
<form action="registro_boleto.php" method="post">
	<table>
		<tr>
			<td><p>Boleto:</p></td> 
	    	<td><p><input type="text" name="clave_boleto" size="4" maxlength="4" value="<?php if (isset($_POST['clave_boleto'])) echo $_POST['clave_boleto']; ?>" /></p></td>
		</tr>
		<tr>
